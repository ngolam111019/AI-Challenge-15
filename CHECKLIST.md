...
- [x] **Frontend Template (`views/tenants/list.njk`)**:
  ...
  - Tích hợp Alpine.js để theo dõi số lượng checkbox được chọn:
    ...
    - Bật nút `Compare Selected` khi số lượng == 2 (Chỉ kích hoạt khi chọn chính xác 2 dòng).
- [x] **HTMX Interactions**:
  - Đảm bảo thứ tự định tuyến API chính xác (`/api/tenants/batch` trước `/api/tenants/:id`) để tránh xung đột ép kiểu tham số.
...
| **TC1.2** | Kiểm tra nút Compare khi chọn 1 dòng | Tích chọn 1 dòng trên bảng. | Nút *Delete Selected* sáng lên, nút *Compare* vẫn mờ với nhãn `(1/2)`. | ✅ |
| **TC1.3** | Hủy xóa nhóm | Bấm *Delete Selected* -> Bấm *Hủy* trên popup confirm. | Bảng giữ nguyên, không có dữ liệu nào bị xóa. | ✅ |
| **TC1.4** | Xóa nhóm thành công | Bấm *Delete Selected* -> Bấm *Đồng ý*. | Các dòng biến mất mượt mà, gọi thành công `/api/tenants/batch` không lỗi NaN. | ✅ |
| **TC1.5** | Điều hướng nút Compare | Tích chọn chính xác 2 dòng (ID 1 và 2) -> Bấm *Compare Selected (2/2)*. | Chuyển hướng sang `/tenants/diff?id1=1&id2=2`. | ✅ |
...
- [x] **Frontend Template (`views/tenants/edit.njk`)**:
  ...
  - Logic phân biệt Mode: Khóa ô *Slug* ở chế độ Edit.
- [x] **HTMX & Alpine Interactions**:
  - Quản lý chuyển tab mượt mà không tải lại trang.
  - Tự động sinh Slug từ Name khi ở chế độ Create New.
  - Tính năng thêm/xóa dòng động cho Approval Tiers và Custom Fields.
  - Tự động hiển thị popup nhập **Change Note / Commit Message** bằng SweetAlert2 khi lưu cấu hình chỉnh sửa.
  - Khởi tạo đầy đủ cấu hình mặc định (Default System Template) cho tất cả các sự kiện và kênh thông báo.
...
- [x] **Frontend Template (`views/tenants/diff.njk`)**:
...
- [x] **Frontend Template (`views/tenants/history.njk`)**:
...
- [x] **Frontend Template (`views/tenants/preview.njk`)**:
...
- [x] **HTMX Interactions**:
  ...
  - Tích hợp nút bấm thực thi tường minh `🚀 Execute Verification Pipeline` với SweetAlert2 kiểm chứng bắt buộc trước khi gọi API mô phỏng.
...
| **TC5.2** | Tương tác Auto-Approve | Nhập số tiền $15,000 (Nhỏ hơn 20k của SafeGuard) -> Bấm *Execute Pipeline*. | Cột bên phải xuất hiện huy hiệu `Auto-Approve ⚡`. | ✅ |
| **TC5.3** | Tương tác Tiered Approval | Kéo thanh trượt số tiền lên $50,000 -> Bấm *Execute Pipeline*. | Cột bên phải hiển thị huy hiệu `Tiered Approval: assessor`. | ✅ |
| **TC5.4** | Cảnh báo dữ liệu bắt buộc | Để trống ô *Employee ID* đánh dấu bắt buộc -> Bấm *Execute Pipeline*. | Xuất hiện SweetAlert2 báo lỗi yêu cầu điền dữ liệu, pipeline không chạy. | ✅ |
| **TC5.5** | Đổi Tenant động | Chuyển dropdown sang GovHealth. | Lược đồ tham số bên trái đổi form hiện *Department* và *Budget Code*. | ✅ |
...
- [x] **Hàm & API Lõi (`processClaim(tenantId, claimData)`)**:
